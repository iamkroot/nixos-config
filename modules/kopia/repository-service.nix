...
let
  ...
  azureSubmodule = lib.types.submodule {
    options = {
      ...
      storageKeyFile = lib.mkOption {
        ...
        default = null;
        description = ''
          Path to a file containing the Azure storage account key.
          Read at runtime for secrets management.
        '';
      };

      sasTokenFile = lib.mkOption {
        type = with lib.types; nullOr str;
        default = null;
        description = ''
          Path to a file containing the Azure SAS token.
          Read at runtime for secrets management.
        '';
      };
      ...
      prefix = lib.mkOption {
        ...
        default = null;
        description = ''
          Prefix to use for objects in the container.
        '';
      };

      storageDomain = lib.mkOption {
        type = with lib.types; nullOr str;
        default = null;
        description = ''
          Azure storage domain (default: blob.core.windows.net).
        '';
      };
    };
  };
...
in
{
      ...
      lib.types.submodule {
        options = {
          ...
          extraConnectArgs = lib.mkOption {
            ...
            default = [ ];
            description = ''
              Extra command line arguments passed to `kopia repository connect`.
            '';
          };
          ...
          extraCreateArgs = lib.mkOption {
            ...
            default = [ ];
            description = ''
              Extra command line arguments passed to `kopia repository create`.
            '';
          };
        };
      }
  ...
  config =
    let
      ...
      mkConnectOrCreate = kopiaExe: backendType: argsVar: connectExtra: createExtra: ''
        if ! ${kopiaExe} repository connect ${backendType} ''$${argsVar} ${connectExtra}; then
          ${kopiaExe} repository create ${backendType} ''$${argsVar} ${createExtra}
        fi
      '';
    in
    lib.mkIf (cfg.backups != { }) {
      assertions = lib.flatten (
        lib.mapAttrsToList (
          ...
          in
          lib.optionals (backup.repository ? azure) (
            let
              azure = backup.repository.azure;
            in
            [
              {
                assertion = azure.storageKeyFile != null || azure.sasTokenFile != null;
                message = "${prefix}: at least one of repository.azure.storageKeyFile or repository.azure.sasTokenFile must be set";
              }
              {
                assertion = azure.storageKeyFile == null || azure.sasTokenFile == null;
                message = "${prefix}: repository.azure.storageKeyFile and repository.azure.sasTokenFile are mutually exclusive";
              }
            ]
          )
      ...
      systemd.services = lib.mapAttrs' (
        ...
        let
          ...
          connectExtra = lib.concatStringsSep " " (map lib.escapeShellArg backup.extraConnectArgs);
          createExtra = lib.concatStringsSep " " (map lib.escapeShellArg backup.extraCreateArgs);

          mkScriptBody =
            if repo ? filesystem then
              ''
                REPO_ARGS="--path ${lib.escapeShellArg repo.filesystem.path}"
                ${mkConnectOrCreate kopiaExe "filesystem" "REPO_ARGS" connectExtra createExtra}
              ''
            else if repo ? azure then
              let
                azure = repo.azure;
              in
              ''
                ${mkCredentialFileExport {
                  varName = "AZURE_STORAGE_KEY";
                  valueFile = azure.storageKeyFile;
                }}
                ${mkCredentialFileExport {
                  varName = "AZURE_STORAGE_SAS_TOKEN";
                  valueFile = azure.sasTokenFile;
                }}
                REPO_ARGS="--container ${lib.escapeShellArg azure.container} --storage-account ${lib.escapeShellArg azure.storageAccount}"
                ${lib.optionalString (azure.prefix != null) ''
                  REPO_ARGS="$REPO_ARGS --prefix ${lib.escapeShellArg azure.prefix}"
                ''}
                ${lib.optionalString (azure.storageDomain != null) ''
                  REPO_ARGS="$REPO_ARGS --storage-domain ${lib.escapeShellArg azure.storageDomain}"
                ''}
                ${mkConnectOrCreate kopiaExe "azure" "REPO_ARGS" connectExtra createExtra}
              ''
              ...
              ''
                ...
                ''}
                ${mkConnectOrCreate kopiaExe "s3" "REPO_ARGS" connectExtra createExtra}
              ''
              ...
              ''
                ...
                ''}
                ${mkConnectOrCreate kopiaExe "sftp" "REPO_ARGS" connectExtra createExtra}
              ''
            else
              let
                dav = repo.webdav;
              ...
              ''
                ...
                ${mkConnectOrCreate kopiaExe "webdav" "REPO_ARGS" connectExtra createExtra}
              '';
